...
class BetController{
    ...
    addAmount(amount, userCallback){
        User.updateOne({_id: this.user['_id']}, {amount: this.user.amount + Number(amount)}, (err) => {
            if(err){
                userCallback({
                    success: false,
                    status: common.getStatusCode(err),
     ...
     * @return none
     */
    static index(after, limit, indexCallback){
        if(!limit){
        ...
        Event.find({ createdTime : { $lte: after}}, 'team1_name team2_name team1_odd team2_odd closingTime', { limit: limit, sort: '-createdTime' }, (err, docs) => {
            if(err){
                indexCallback({
                    success: false,
                    status: common.getStatusCode(err),
